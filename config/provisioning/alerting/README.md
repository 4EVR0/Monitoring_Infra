...
| 파일 | 역할 |
|------|------|
| `contact-points.yaml` | Discord 연결처. 웹훅 URL은 `${DISCORD_ALERT_WEBHOOK_URL}` 참조, 메시지는 templates 참조 |
| `notification-policy.yaml` | 모든 알림 → discord 라우팅 |
| `rules.yaml` | 알림 룰 8종 |
| `templates.yaml` | Discord 메시지 디자인(스타일 B) — 제목·본문·링크 |
